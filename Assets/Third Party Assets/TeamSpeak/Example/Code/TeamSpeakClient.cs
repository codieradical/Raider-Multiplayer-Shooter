...
public class TeamSpeakClient
{
    ...
    public void StopClient()
    {
        if (started)
        {
            ...
            if (logErrors)
            {
                Debug.Log("TeamSpeak client successfully stopped: " + this);
            }
        }
    }
    ...
    public string GetTransferFileRemotePath(anyID transferID)
    { //The returned memory is dynamically allocated, remember to call ts3client_freeMemory() to release it
        string result = null;
        if (started)
        {
            IntPtr strPtr = IntPtr.Zero;
    ...
    public bool IsTransferSender(anyID transferID)
    { //1 == upload, 0 == download
        bool result = false;
        if (started)
        {
    ...
    public float GetCurrentTransferSpeed(anyID transferID)
    { //bytes/second within the last few seconds
        float result = 0;
        if (started)
        {
    ...
    public float GetAverageTransferSpeed(anyID transferID)
    { //bytes/second since start of the transfer
        float result = 0;
        if (started)
        {
